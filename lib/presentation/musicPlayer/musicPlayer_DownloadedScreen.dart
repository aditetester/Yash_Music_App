...
class MusicPlayerDownloadedScreen extends StatefulWidget {
  ...
  @override
  State<MusicPlayerDownloadedScreen> createState() => _MusicPlayerScreenState();
}

class _MusicPlayerScreenState extends State<MusicPlayerDownloadedScreen> {
  ...
  Widget build(BuildContext context) {
    return Observer(
      builder: (context) {
        final recentMusic = _musicControllerStore.recentDownloadedMusicPlay;
                      ...
                      children: [
                        ...
                        Container(
                          ...
                          child: Padding(
                            ...
                            child: ClipOval(
                              child: Hero(
                                tag: recentMusic.id.toString(),
                                child: FadeInImage(
                                  placeholder: AssetImage('assets/icon/icon.png'),
                                  image: NetworkImage(recentMusic.image ?? ''),
                                  fit: BoxFit.cover,
                                  height: 37.h,
                                  width: 83.w,
                                ),
                        ...
                        Container(
                          ...
                          child: Center(
                            child: Text(
                              recentMusic.title ?? 'No Song Playing',
                              style: TextStyle(
                                fontSize: 20,
                                fontWeight: FontWeight.bold,
                              ),
                        ...
                        Observer(
                          builder: (_) {
                            var currentPosition = _musicControllerStore.currentPosition;
                            var totalDuration = _musicControllerStore.totalDuration;

                            if (_formatDuration(currentPosition) == _formatDuration(totalDuration)) {
                                      ...
                                      GestureDetector(
                                        onTapDown: (TapDownDetails details) {
                                          ...
                                          final newPosition = Duration(
                                            milliseconds: (tappedPercentage * totalDuration.inMilliseconds).toInt(),
                                          );
                            ...
                            IconButton(
                              ...
                              onPressed: () async {
                                ...
                                _musicControllerStore.playDownloadPrevious(
                                  currentIndex: (_musicControllerStore.getCurrentMusicIndex == 0)
                                      ? (_musicControllerStore.AllDownloadedMusic!.length - 1)
                                      : _musicControllerStore.getCurrentMusicIndex - 1,
                                  previoudPlay: _musicControllerStore.AllDownloadedMusic![
                                    (_musicControllerStore.getCurrentMusicIndex == 0)
                                        ? (_musicControllerStore.AllDownloadedMusic!.length - 1)
                                        : _musicControllerStore.getCurrentMusicIndex - 1
                                  ],
                                );
                            ...
                            Container(
                              ...
                              child: Observer(
                                builder: (_) => IconButton(
                                  icon: _musicControllerStore.isPlaying
                                      ? SvgPicture.asset(
                                  ...
                                  color: Colors.white,
                                  iconSize: 40,
                                  onPressed: () => _playPause(recentMusic.audio ?? ''),
                                ),
                              ),
                            ),
                            IconButton(
                              ...
                              onPressed: () {
                                _musicControllerStore.playDownloadNext(
                                  currentIndex: (_musicControllerStore.AllDownloadedMusic!.length - 1 ==
                                          _musicControllerStore.getCurrentMusicIndex)
                                      ? 0
                                      : _musicControllerStore.getCurrentMusicIndex + 1,
                                  nextplay: _musicControllerStore.AllDownloadedMusic![
                                    (_musicControllerStore.AllDownloadedMusic!.length - 1 ==
                                            _musicControllerStore.getCurrentMusicIndex)
                                        ? 0
                                        : _musicControllerStore.getCurrentMusicIndex + 1
...
class WaveformPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final pattern = [
      0.1, 0.1, 0.3, 0.2, 0.1, 0.3, 0.5, 0.8, 0.4, 0.2,
      0.4, 0.8, 0.4, 0.2, 0.3, 0.6, 0.5, 0.7, 0.4, 0.2,
    ];
    ...
    for (int i = 0; i < barCount; i++) {
      ...
      final height = heightFactor * size.height * 0.8;
